...
import { PRODUCTS, BLOGS, COUPONS, FAQS, INITIAL_REVIEWS } from "../../shared/types/data";
import { DEFAULT_BUSINESS_INFO } from "../../shared/types/businessConfig";
import { TOP_LEVEL_CATEGORIES, CROSS_CUTTING_COLLECTIONS } from "../../shared/types/taxonomy";
import { DEFAULT_NAVIGATION_CONFIG } from "../../shared/types/navConfig";
import { DEFAULT_FESTIVAL_CAMPAIGNS } from "../../shared/types/campaignConfig";

dotenv.config();

const SECRET_KEY = process.env.JWT_SECRET || "blackfawn_secret_key_2026";
const app = express();
...
app.use((req, res, next) => {
  const allowedOrigins = [
    process.env.STORE_URL || "http://localhost:5173",
    process.env.ADMIN_URL || "http://localhost:5174",
    "http://localhost:5173",
    "http://localhost:5174",
    "https://blackfawn.in",
    "https://www.blackfawn.in",
    "https://admin.blackfawn.in"
  ];
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[BLACKFAWN Server] Enterprise API backend active on http://localhost:${PORT}`);
  });
}
